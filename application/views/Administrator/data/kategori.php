...
table = $('#table_kategori').DataTable({
    'ajax': {
        'url': '<?= base_url('administrator/kategori_fetch') ?>',
        'dataSrc': 'data',
        'type': 'POST'
    },
    'processing': true,
...
function edit(id) {
    ...
    $.ajax({
        url: "<?= base_url("administrator/kategori_edit/") ?>" + id,
        dataType: "html",
        success: function(result) {
            $.LoadingOverlay("hide");
...
function hapus(id) {
    Swal.fire({
        ...
        if (result.value) {
            ...
            $.post('<?= base_url('administrator/kategori_hapus') ?>', {
                'id': id
...
function detail(id) {
    $.LoadingOverlay("show");
    $.ajax({
        url: "<?= base_url("administrator/kategori_detail/") ?>" + id,
        dataType: "html",
        success: function(result) {
            $.LoadingOverlay("hide");
            $('#infouser').modal('show');
...
<!-- modal tambah -->
<div class="modal fade" tabindex="-1" role="dialog" id="tambah_kategori" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <form id="form_tambah" action="<?= base_url('administrator/kategori_simpan'); ?>" method="post">
...
<div class="modal fade" tabindex="-1" role="dialog" id="update_buku" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <form id="form_update" action="<?= base_url('administrator/kategori_update'); ?>" method="post">
...
<div class="modal" id="infouser" role="dialog" data-backdrop="false" style="background: rgba(0,0,0,0.3);">
    <div class="modal-dialog modal-md">
        <div class="modal-content P-1">
            ...
            <form id="form_detail" action="<?= base_url('date_perpus/kategori_detail'); ?>" method="post">

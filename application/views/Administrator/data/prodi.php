...
table = $('#table_prodi').DataTable({
    'ajax': {
        'url': '<?= base_url('administrator/prodi_fetch') ?>',
        'dataSrc': 'data',
        'type': 'POST'
    },
    'processing': true,
...
function edit(id) {
    ...
    $.ajax({
        url: "<?= base_url("administrator/prodi_edit/") ?>" + id,
        dataType: "html",
        success: function(result) {
            $.LoadingOverlay("hide");
...
function hapus(id) {
    Swal.fire({
        ...
        if (result.value) {
            ...
            $.post('<?= base_url('administrator/prodi_hapus') ?>', {
                'id': id
...
function detail(id) {
    $.LoadingOverlay("show");
    $.ajax({
        url: "<?= base_url("administrator/prodi_detail/") ?>" + id,
        dataType: "html",
        success: function(result) {
            $.LoadingOverlay("hide");
            $('#infouser').modal('show');
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Masukkan Data Prodi</h5> <button type="button" class="close"
                    data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
            </div>
            <form id="form_tambah" action="<?= base_url('administrator/prodi_simpan'); ?>" method="post">
...
<div class="modal fade" tabindex="-1" role="dialog" id="update_buku" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Perbarui Data Prodi</h5> <button type="button" class="close"
                    data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
            </div>
            <form id="form_update" action="<?= base_url('administrator/prodi_update'); ?>" method="post">

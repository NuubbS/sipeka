...
table = $('#table_anggota').DataTable({
    'ajax': {
        'url': '<?= base_url('administrator/member_fetch') ?>',
        'dataSrc': 'data',
        'type': 'POST'
    },
    'processing': true,
...
function edit(id) {
    ...
    $.ajax({
        url: "<?= base_url("administrator/member_edit/") ?>" + id,
        dataType: "html",
        success: function(result) {
            $.LoadingOverlay("hide");
            $('#update_anggota').modal('show');
...
function hapus(id) {
    Swal.fire({
        ...
        if (result.value) {
            ...
            $.post('<?= base_url('administrator/member_hapus') ?>', {
                'id': id
...
<!-- modal tambah -->
<div class="modal fade" tabindex="-1" role="dialog" id="tambah_anggota" aria-hidden="true" style="display: none;">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Masukkan Data Anggota</h5> <button type="button" class="close"
                    data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
            </div>
            <form id="form_tambah" action="<?= base_url('administrator/member_simpan'); ?>" method="post">
...
<!-- modal tambah-->
<!-- modal update -->
<div class="modal fade" tabindex="-1" role="dialog" id="update_anggota" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Perbarui Data Anggota</h5> <button type="button" class="close"
                    data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
            </div>
            <form id="form_update" action="<?= base_url('administrator/member_update'); ?>" method="post">

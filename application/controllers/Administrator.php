...
class Administrator extends CI_Controller {
    ...
    $this->datatables_builder->select('tb_buku.judul, tb_prodi.prodi, tb_rak.rak_nama, tb_buku.date_created,  tb_buku.buku_id');
    ...
    $this->datatables_builder->join('tb_prodi', 'tb_buku.prodi_id = tb_prodi.prodi_id');
    $this->datatables_builder->join('tb_rak', 'tb_buku.rak_id = tb_rak.rak_id');
    $this->datatables_builder->where_in('tb_buku.kategori_id', ['5']);
    $this->datatables_builder->order_by('tb_buku.buku_id', 'desc');
    $m = $this->datatables_builder->get();
    $this->datatables_builder->render_no_keys($m);
    }
...
{
$this->datatables->search('tb_user.user_id, tb_user.nama, tb_user.alamat, tb_user.no_handphone, tb_user.date_created, tb_user.date_updated');
$this->datatables->select('tb_user.user_id, tb_user.nama, tb_user.alamat, tb_user.no_handphone, tb_user.date_created, tb_user.date_updated');
$this->datatables->from('tb_user');
$this->datatables->where('level_id', '2');
$this->datatables->order_by('user_id', "ASC");
$m = $this->datatables->get();
$no = 1;
foreach ($m as $key => $value) {
$act = '';
$act .= sprintf('<button onclick="detail(%s)" class="btn btn-icon btn-sm btn-info m-1" data-toggle="tooltip" data-placement="top" title="Detail buku"><i class="fas fa-user-edit"></i></button>', $value['user_id']);
// $act .= sprintf('<button onclick="edit(%s)" class="btn btn-icon btn-sm btn-primary m-1" data-toggle="tooltip"
//     data-placement="top" title="Edit buku"><i class="fas fa-user-edit"></i></button>', $value['user_id']);
// $act .= sprintf('<button onclick="hapus(%s)" class="btn btn-icon btn-sm btn-danger m-1" data-toggle="tooltip"
//     data-placement="top" title="Hapus buku"><i class="fas fa-trash-alt"></button>', $value['user_id']);
$m[$key]['as'] = $act;
$m[$key]['user_id'] = $no;
$no++;
}
...
{
$this->datatables_builder->search(explode(', ', 'email, nama, password, no_handphone, alamat, date_created, date_updated'));
$this->datatables_builder->select('email, nama as nama, password, no_handphone, alamat, date_created, date_updated');
$this->datatables_builder->from('tb_user');
$this->datatables_builder->order_by('tb_user.date_created', 'desc');
$m = $this->datatables_builder->get();
$this->datatables_builder->render_no_keys($m);
}
